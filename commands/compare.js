...
function extractIdsFromFile(filePath, prefix, numberLength) {
    ...
    visitCallExpression(path) {
      ...
      if (isTest) {
        ...
        if (match) {
          ...
          foundIds.push({ id, title, file: filePath });
        }
      }
...
export default async function main(cliOptions = {}) {
  try {
    ...
    const idMap = {}; // id -> [{file, title}]
    for (const file of files) {
      const ids = extractIdsFromFile(file, prefix, numberLength);
      for (const { id, title, file: f } of ids) {
        if (!idMap[id]) idMap[id] = [];
        idMap[id].push({ file: f, title });
      }
    }

    const duplicates = Object.entries(idMap).filter(([_, occurrences]) => occurrences.length > 1);

    if (duplicates.length > 0) {
      console.log('\n🚨 Duplicate test IDs found:\n');
      for (const [id, occurrences] of duplicates) {
        console.log(`ID >>> ${id} <<< appears in:`);
        for (const { file, title } of occurrences) {
